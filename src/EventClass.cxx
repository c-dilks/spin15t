...
Bool_t EventClass::Valid(Int_t idx)
{
  ...
  else if(idx==kPi0)
  {
    if( fabs(N12-2)<0.1 &&
        Z<0.8 &&
        CheckMass(M12) ) return true;
  }

  // three or more photons cuts
  else if(idx==kThr)
  {
    if( N12>2 ) return true;
  }
  ...
  else if(idx==kEtm)
  {
    if( fabs(N12-2)<0.1 &&
        ...
        FiducialGeom(Eta,Phi,1.5) ) return true;
  };
  return false;
};
//////////////////////////
...
Bool_t EventClass::FiducialGeom(Double_t Eta_, Double_t Phi_, Double_t Cd)
{
  ...
  Xd = (Zd*tan(Theta))/sqrt(1+pow(tan(Phi_),2));
  Yd = (Zd*tan(Theta)*tan(Phi_))/sqrt(1+pow(tan(Phi_),2));
  if(fabs(Phi_)<3.1415/2.0) Xd*=-1;
